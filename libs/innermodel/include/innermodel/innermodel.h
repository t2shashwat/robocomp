...
class InnerModel
{
	...
	public:
		...
		InnerModelNode *getNode(const QString & id) const { if (hash.contains(id)) return hash[id]; else return nullptr;}   //DEPRECATED
		
		template <typename N> N* getNode(const std::string &id) const
		{
			const QString &qid = QString::fromStdString(id);
			auto rnode = hash.value(qid, nullptr);
			if(rnode == nullptr)
				...
				{ 
					throw RoboComp::BadCastToFinalType("Can't convert type");
				}
		}
